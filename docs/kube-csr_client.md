...
kube-csr client my-app --generate

# generate the private key, the csr and then submit the csr
kube-csr client my-app --generate --submit

# generate the private key, the csr, submit and approve the csr
kube-csr client my-app --generate --submit --approve

# Generate the private key, the csr, submit, approve and fetch the csr
kube-csr client my-app --generate --submit --approve --fetch
kube-csr client my-app -gsaf
kube-csr client my-app -gsaf --subject-alternative-names 192.168.1.1,etcd-0.default.svc.cluster.local

# Generate the private key, the csr, submit and fetch the csr when externally approved
kube-csr client my-app --generate --submit --fetch --fetch-interval 10s --fetch-timeout 10m

# Generate the private key, the csr, submit, approve, fetch and delete the csr
kube-csr client my-app --generate --submit --approve --fetch --delete 

# Generate the private key, the csr, submit, approve and fetch the csr. Override any existing and use a kubeconfig
kube-csr client my-app -gsaf --override --kubeconfig-path ~/.kube/config

# Execute all steps with a custom kubernetes csr name
kube-csr client skydns --csr-name kv-etcd -gsafd --override --kubeconfig-path ~/.kube/config
